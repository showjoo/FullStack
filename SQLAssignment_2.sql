...
--     ---------------------------------            ----------------------------------
SELECT DISTINCT su.CompanyName as [Supplier Company Name], s.CompanyName as [Shipping Company Name]
FROM dbo.Orders o JOIN dbo.[Order Details] od on o.OrderID = od.OrderID JOIN dbo.Shippers s on s.ShipperID = o.ShipVia JOIN dbo.Products p on p.ProductID = od.ProductID JOIN dbo.Suppliers su on su.SupplierID = p.SupplierID
...
SELECT * 
FROM Employees
WHERE Title LIKE '%Manager%' and EmployeeID IN (
    SELECT ReportsTo
    FROM Employees
    GROUP BY ReportsTo
    HAVING Count(*) > 2
...
-- I don't understand what does 'Name' columns do here, so I just include ContactName
